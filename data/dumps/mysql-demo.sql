...
INSERT INTO page (parent, name, active, created_by, create_date, changed_by, change_date, deleted)
VALUES ((SELECT x.id FROM (SELECT * FROM page) x where x.name='first'), 'second', true, (SELECT x.id FROM (SELECT * FROM `user`) x where x.login='admin'), '20121001 22:00:00', (SELECT x.id FROM (SELECT * FROM `user`) x where x.login='admin'), '20121001 22:00:00', false);

INSERT INTO page (parent, name, active, created_by, create_date, changed_by, change_date, deleted)
VALUES ((SELECT x.id FROM (SELECT * FROM page) x where x.name='second'), 'third', true, (SELECT x.id FROM (SELECT * FROM `user`) x where x.login='admin'), '20121001 22:00:00', (SELECT x.id FROM (SELECT * FROM `user`) x where x.login='admin'), '20121001 22:00:00', false);
